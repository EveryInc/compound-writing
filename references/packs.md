...
## What a pack changes in an editing step, measured

One measurement so far, against one house editor's recorded copy edits (kate bench, September 2026, `docs/learnings/2026-09-11-kate-bench-pack-integration.md`). A pack's rules changed what `cw-line-edit` did *at* a sentence — fewer edits, more paragraphs left alone, a higher share of edits where the editor also edited — and did not change which sentences it found: recall of the editor's exact edits stayed within one of sixty across two new rule families, a pack-authority change, deterministic checks, and retrieved examples, each cited on most paragraphs. Giving the step the whole draft instead of three paragraphs was worth more than any of them. Read a pack as the way a house makes a general step edit its prose *less and more precisely*; a house that wants the step to find more of its editor's edits needs a mechanism this layer does not have yet. Two hypotheses that measured no gain (`checks/` run before judgment; `examples/` retrieved per paragraph) are kept with tests on the branch `cursor/kate-bench-pack-integration-arms-cf0e` for anyone re-measuring against a different editor or a larger store.
