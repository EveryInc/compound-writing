#!/usr/bin/env python3
"""Resolve the Compound Packs declared by a writing home into pack roots.

Reads the `packs:` list from `<home>/.compound-writing/config.yaml` and
`config.local.yaml` (both layers concatenate; local adds, never replaces),
...
cache: it parses both config layers, shape-checks each entry, and prints
    ...
    {"declared": <bool|null>, "entries": <n>, "errors": [...], "home": ...}
...
writing home -- and so no config -- could be located.

Entry shape (documented subset -- anything else under `packs:` is a loud error):

    packs:
      - source: compound-packs/house-style     # home-relative path
      - source: ~/compound-packs/every-voice   # ~ or absolute path
      - source: https://github.com/o/r         # git URL: ref required
        ref: v1.2.0                            # tag, sha, or branch
        path: packs                            # optional subfolder (git only)
        pack: [voice, style]                   # one id, a list, or omit = all
        id: house-voice                        # rename (single-pack entries)
      - source: https://github.com/o/r/tree/main/packs   # tree-URL sugar

Git sources cache under `<scratch-root>/cw-packs/<sha256(url\\nref)>` with an
...
skipped); path sources still resolve. Environment overrides:
CW_PACKS_CACHE_ROOT (cache base for tests), CW_PACKS_GIT_TIMEOUT (seconds,
default 60).
...
GIT_TIMEOUT = float(os.environ.get("CW_PACKS_GIT_TIMEOUT") or 60)

CONFIG_DIR = ".compound-writing"
CONFIG_FILES = ("config.yaml", "config.local.yaml")
GUIDE = "references/packs.md"
...
def cache_base() -> str | None:
    configured = os.environ.get("CW_PACKS_CACHE_ROOT")
    if configured:
...
def enumerate_packs(source_root: str, boundary: str, escaped: list, self_name: str | None = None) -> dict:
    ...
    within (the git checkout, the writing home, or the source root itself); a
...
def nested_rules_warning(pack_id: str, pack_dir: str, boundary: str) -> str | None:
    ...
    return (
        ...
        f" never reads -- move rules to the pack's top level (see {GUIDE}, Pack layout)"
    )
...
def resolve_entry(entry: dict, home_root: str, roots: list, warnings: list, errors: list) -> None:
    ...
    if _is_git_url(source):
        ...
        if os.path.isabs(expanded):
            ...
            source_root = os.path.realpath(os.path.join(home_root, expanded))
            home_real = os.path.realpath(home_root)
            if not _within(source_root, home_real) or _within(source_root, os.path.join(home_real, ".git")):
                errors.append(f"{label}: home-relative source `{source}` resolves outside the writing home")
                return
            boundary = home_real
        if not os.path.isdir(source_root):
...
def _emit(declared_only: bool, entries: list, roots: list, warnings: list, errors: list,
          home: str | None, declared: bool | None = None) -> int:
    if declared_only:
        print(json.dumps({"declared": declared, "entries": len(entries), "errors": errors, "home": home}))
    else:
        print(json.dumps({
...
def _main(argv: list) -> int:
    parser = argparse.ArgumentParser(description="Resolve the Compound Packs declared by a writing home.")
    parser.add_argument(
        "--home", default=None, metavar="PATH",
        help="file or directory to start the writing-home search from (default: the working directory)",
    )
    parser.add_argument(
    ...
    if args.declared_only:
        ...
        return _emit(True, entries, roots, warnings, errors, home_root, declared=bool(entries or errors))

    for entry in entries:
        try:
            resolve_entry(entry, home_root, roots, warnings, errors)
    ...
    # CONFIG_FILES, so a personal pack can never displace the shared one.
    by_id = {}
    final = []
    for root in roots:
        ...
        by_id[root["id"]] = root
        final.append(root)
    return _emit(False, entries, final, warnings, errors, home_root)
...
def main() -> int:
    try:
        ...
        print(json.dumps({
            "roots": [], "warnings": [], "errors": [f"packs resolver failed unexpectedly: {exc}"],
            "entries": 0, "home": None,
        }))
        return 0
