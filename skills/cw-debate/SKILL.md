---
name: cw-debate
description: Run a multi-round deliberation between reviewers. Unlike /cw-panel (which synthesizes), /cw-debate has reviewers respond to each other's arguments across rounds until tensions resolve or reach acknowledged stalemate.
---
...
## When to Use /cw-debate vs /cw-panel

| Use | When |
|-----|------|
| `/cw-panel` | You want multiple perspectives synthesized. Fast. Tensions surfaced for you to decide. |
| `/cw-debate` | You want reviewers to actually argue it out. More thorough. Tensions may resolve through deliberation. |

**Choose /cw-debate when:**
...
Same as /cw-panel—all reviewers analyze the draft independently.
...
Read `../../references/context-contract.md`, the draft, and any available brief, outline, project instructions, feedback, or status metadata. Establish:
- piece_type
- audience
- stage
- goals
...
Same as /cw-panel—propose 4-6 reviewers based on context.
...
- reader + hitchcock (orientation vs. mystery)
...
Run all reviewers in parallel when the active platform supports isolated workers and the user's explicit debate request authorizes them. On Claude, use the matching reviewer agents when available; on Codex or another runtime, use the reviewer skills or isolated workers with those lenses. Collect independent outputs.
...
| reader + hitchcock | Orientation vs. Mystery | Distinguishing strategic withholding from accidental confusion |
| reader + hemingway | Context vs. Brevity | Deciding what setup a first-time reader needs |
| reader + sorkin | Orientation vs. Momentum | Giving readers footing without slowing the piece |
...
[Reads draft]
[Loads the brief and project context: personal essay, general audience, revision stage]
...
| **reader** | Trace the first-time reading experience |
...
Debate is materially slower and more context-intensive than panel because reviewers receive follow-up challenges. Use it for consequential tensions, cap the number of rounds, and prefer `cw-panel` for routine coverage.
...
## Relationship to /cw-panel

`/cw-panel` and `/cw-debate` share infrastructure:
- Same reviewer agents
- Same context loading from the active project and artifact
- Same panel proposal logic

The difference:
- `/cw-panel` → synthesizer (one pass, tensions surfaced)
- `/cw-debate` → moderator (multiple rounds, tensions engaged)

A natural workflow: run `/cw-panel` first for quick feedback, then `/cw-debate` on specific tensions if you want them argued out.
